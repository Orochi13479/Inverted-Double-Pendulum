...
double torqueRamp(double current_time, double duration, double max_torque) {
    double slope = max_torque / duration;
    return slope * current_time;
}
...
int main(int argc, char** argv) {
    ...
    double current_time = 0.0; // in seconds
    const double torque_ramp_duration = 10.0; // Duration in seconds for torque ramp
    ...
    while (!ctrl_c_pressed) {
        ...
        while(current_time < torque_ramp_duration && !ctrl_c_pressed){
            std::chrono::high_resolution_clock::time_point now = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> elapsed_seconds = now - start_time;
            current_time = elapsed_seconds.count(); // Convert to seconds

            // Calculate torque ramp
            double torque_ramp_value = torqueRamp(current_time, torque_ramp_duration, 2.0); // Max torque set to 2.0 Nm

            // Update torque commands
            torque_command[0] = tau(0) * torque_ramp_value;
            torque_command[1] = tau(1) * torque_ramp_value;
        }
